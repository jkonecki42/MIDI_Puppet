...
struct file_hdr_t
{                             // the optional bytestream file header
    char id1;                 // 'P'
    char id2;                 // 't'
...
#endif
        switch (timer_num)
        { // All timers are put in CTC mode
...
#if DBUG
        Serial.print("header: volume_present=");
        Serial.println(volume_present);
#endif
...
{ // **** TIMER 1
...
#if TESLA_COIL
        if (*timer1_pin_port & timer1_pin_mask)
    ...
    if (doing_delay && delay_toggle_count)
        --delay_toggle_count; // countdown for tune_delay()
}
...
#if TESLA_COIL
    if (*timer3_pin_port & timer3_pin_mask)
...
#if TESLA_COIL
    if (*timer4_pin_port & timer4_pin_mask)
        teslacoil_rising_edge(4); // do a tesla coil pulse
#endif
}
#endif
...
#if TESLA_COIL
    if (*timer5_pin_port & timer5_pin_mask)
        teslacoil_rising_edge(5); // do a tesla coil pulse
#endif
}
#endif